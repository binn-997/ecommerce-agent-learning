...
CI（`.github/workflows/ci.yml`，Python 3.12）运行 `ruff check amazon_ai_platform tests`、`pytest` 和 `python -m amazon_ai_platform.listing_agent`（listing_agent 自带 `__main__` 离线 demo，输出 JSON 草稿）。
...
`examples/` 只负责组装依赖和演示数据，不承载业务逻辑。已知例外：`05_amazon_ads_client.py`（Amazon Ads Reporting v3 client）和 `06_rag_knowledge_base.py`（离线 RAG 流水线，hash embedder 仅为离线可跑）是自包含演示，逻辑尚未提取进包内——复用其能力时先提取到 `amazon_ai_platform/`，勿在别处复制。`03_llm_gateway.py` 只是 `llm_gateway:app` 的兼容入口。`tests/` mock 外部系统的协议边界（HTTP/LLM），不 mock 被测逻辑本身；`sql/init.sql` 是 PostgreSQL 幂等事实表结构。
...
- 开工前用 `git branch --show-current` 确认分支；只暂存本任务相关路径，不用 `git add .`；未经用户要求不 push、不建 PR。
- 文档统一简体中文（命令、类型名、API 字段保留英文）；行为变更需按 AGENTS.md §11 同步 `README.md`、`docs/PROJECT_OVERVIEW.md` 和相关运行记录。
