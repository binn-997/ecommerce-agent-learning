...
class Backend:
    ...
    async def search_policy(self, context, request):
        return GroundedAnswer(status="insufficient_evidence", answer="manual", requires_human_review=True)
...
def service(scopes: set[str]) -> MCPToolService:
    return MCPToolService(AuthContext(
...
def test_tool_arguments_cannot_select_another_seller() -> None:
    result = asyncio.run(service({"sales:read"}).get_sales_metrics(SalesMetricsInput(
...
def test_missing_scope_is_explicitly_denied_and_audited() -> None:
    ...
    with pytest.raises(ToolAccessDenied, match="inventory:read"):
        asyncio.run(tools.get_inventory_risk(InventoryRiskInput(
            sku="SYNTHETIC", as_of=date(2026, 7, 16)
        )))
    assert tools.audit_log[-1]["outcome"] == "denied"


def test_policy_tool_refuses_without_evidence() -> None:
    result = asyncio.run(service({"policy:read"}).search_policy(PolicySearchInput(
...
def test_listing_tool_can_only_return_human_review_draft() -> None:
    ...
    result = asyncio.run(service({"listing:draft"}).draft_listing(ListingDraftInput(
        brief=brief, request_id="draft-001"
    )))
