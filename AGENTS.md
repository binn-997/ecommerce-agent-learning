...
2. `docs/PROJECT_OVERVIEW.md`：模块边界、数据流、运行与验收入口；
3. `.env.example`：支持的环境变量；
4. 需要修改的模块及对应测试。
...
macOS 使用 Colima 承载 Docker Engine。项目的服务组成、运行命令和验收入口见 `README.md` 与 `docs/PROJECT_OVERVIEW.md`。
...
- 改变模块边界、数据流或验收入口：更新 `docs/PROJECT_OVERVIEW.md`；
- 改变 Dockerfile、Compose、端口、服务名或数据卷：同步更新 `README.md` 和相关运行验收记录；
